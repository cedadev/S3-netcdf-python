...
def _build_list_of_indices(nsf, var_shape, subarray_shape):
    ...
    location = numpy.zeros((nsf, 2, len(subarray_shape),),'i')
...
class CFAfile():
    ...
    """
    def __init__(self, in_nc_ds, filename, max_file_size):
        """
    ...
    def write(self, base_path=None):
        ...
        # if no base is specified then set it to sub_dir
        if base_path == None:
            base_path = sub_dir
        ...
        out_cfa_ds = netCDF4.Dataset(out_file, 'w', format="NETCDF4", clobber=True)
        ...
        for sa in self._variables:
            ...
            # add the dimensions
            in_var_atts["cf_dimensions"] = " ".join(in_var.dimensions)
            # modify the base_path
            sa._cfa_meta_data["base"] = base_path
    ...
    def write_subarray(self, varnumber, subarray_number):
        ...
        return var.write(subarray_number, os.path.dirname(sub_dir))
    ...
    def _create_variables_sub_array_metadata(self):
        ...
        if ".nc" in self._filename:
            sub_dir = os.path.basename(self._filename[:-3])
        else:
            sub_dir = os.path.basename(self._filename)
...
class CFAvariable():
    ...
    def write(self, subarray_number, path):
        """
        ...
        out_name = os.path.join(path, sa["subarray"]["file"])
        # create the local file
        out_nc_ds = netCDF4.Dataset(out_name, 'w', format='NETCDF4', clobber=True)   # we always write as NETCDF4 and overwrite
        ...
        for d in range(0, len(in_var.dimensions)):
            ...
            out_dim_var.setncatts({k: in_dim_var.getncattr(k) for k in in_dim_var.ncattrs()})
            # copy the dimension variable data
            out_dim_var[:] = in_dim_var[location[d]:location[d]+shape[d]]
        ...
        out_slice = [slice(location[d],location[d]+shape[d]) for d in range(0, len(shape))]
